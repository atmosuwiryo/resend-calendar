...
export class V4Controller {
  ...
  async listEvents(@Query() queryParams: ListEventsDto): Promise<CalendarEvent[]> {
  ...
  async createEvent(@Body() createEventDto: CreateEventDto): Promise<CalendarEvent> {
  ...
  @ApiResponse({ status: 204, description: 'The calendar event has been deleted' })
  ...
  @HttpCode(HttpStatus.CREATED)
  async quickAdd(@Body() quickAddDto: QuickAddEventDto): Promise<CalendarEvent> {
